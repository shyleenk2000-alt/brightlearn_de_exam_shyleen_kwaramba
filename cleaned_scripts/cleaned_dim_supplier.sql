...
BEGIN
    ...
    -- Insert only new cleaned suppliers
    ...
    SELECT DISTINCT
        LOWER(LTRIM(RTRIM(s.[supplier]))) AS supplier

    FROM [stg_brightlearn_sales].[dbo].[stg_dim_supplier] s

    WHERE s.[supplier] IS NOT NULL
      AND LTRIM(RTRIM(s.[supplier])) <> ''

      -- Prevent duplicate suppliers
      AND NOT EXISTS
      (
          SELECT 1
          FROM [cleaned_brightlearn_sales].[dbo].[cleaned_dim_supplier] c
          WHERE c.[supplier] = LOWER(LTRIM(RTRIM(s.[supplier])))
      );
