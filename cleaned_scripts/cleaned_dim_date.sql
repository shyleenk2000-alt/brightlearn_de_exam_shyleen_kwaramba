...
BEGIN
    ...
    -- Insert only new cleaned transaction dates
    ...
    SELECT DISTINCT
        TRY_CONVERT(DATE, s.[transaction_date], 23)

    FROM [stg_brightlearn_sales].[dbo].[brightlearn_sales_raw_data] s

    WHERE TRY_CONVERT(DATE, s.[transaction_date], 23) IS NOT NULL

    AND NOT EXISTS
    (
        SELECT 1
        FROM [cleaned_brightlearn_sales].[dbo].[cleaned_dim_date] d
        WHERE d.[transaction_date] = TRY_CONVERT(DATE, s.[transaction_date], 23)
    );
